...
class TestTicketiaProactiveAgents(unittest.TestCase):
    ...
    def test_grant_hunter(self):
        with app.app_context():
            ...
            agent.check_new_grants(self.user)
            
            # 3. Check Notification
            notif = Notification.query.filter_by(user_phone=self.user.user_phone, type='grant').first()
    ...
    def test_networker_synergy(self):
        with app.app_context():
            ...
            t1 = Ticket(user_phone=self.user.user_phone, total=100.0, concept="Saco Harina", date=datetime.now())
            db.session.add(t1)
            db.session.commit()
            ...
            agent.run_daily_networking(self.user)
            
            # 4. Check Match & Notification
            match = SynergyMatch.query.first()
            self.assertIsNotNone(match)
            notif = Notification.query.filter_by(user_phone=self.user.user_phone, type='networking').first()
            self.assertIsNotNone(notif)
    ...
    def test_business_coach_projection(self):
        with app.app_context():
            ...
            # Last Month: 1000€
            t_last = Ticket(user_phone=self.user.user_phone, total=1000.0, date=last_month)
            # This Month: 1500€ (Spending more!)
            t_curr = Ticket(user_phone=self.user.user_phone, total=1500.0, date=today)
            ...
            agent.run_daily_analysis(self.user)
            
            # 3. Check Alert
            notif = Notification.query.filter_by(user_phone=self.user.user_phone).first()
            self.assertIsNotNone(notif)
    ...
    def test_post_sales_complaint(self):
        with app.app_context():
            ...
            response, media = agent.handle_inquiry(self.user.user_phone, msg, self.user)
            ...
            notif = Notification.query.filter_by(user_phone=self.user.user_phone, type='alert').first()
            self.assertIsNotNone(notif)
