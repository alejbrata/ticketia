...
def evaluate_agents():
    ...
    # Load Dataset
    try:
        with open('llmops/datasets/gold_standard.json', 'r', encoding='utf-8') as f:
            dataset = json.load(f)
    ...
    with app.app_context():
        ...
        if not user:
            print("⚠️ No user in DB. Creating a mock profile for evaluation.")
            user = BusinessProfile(
                user_phone="000000000",
                business_name="Mock Business",
                email="mock@example.com",
                system_prompt="Eres un asistente IA de pruebas."
            )
